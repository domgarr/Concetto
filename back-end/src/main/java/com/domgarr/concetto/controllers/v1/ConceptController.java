...
import com.domgarr.concetto.api.v1.model.ConceptListDTO;
import com.domgarr.concetto.exception.ForbiddenAccessError;
...
public class ConceptController {
    ...
    @GetMapping("/all")
    public ResponseEntity<ConceptListDTO> getAllConcepts(@AuthenticationPrincipal OAuth2User principal) {
    ...
    public ResponseEntity<List<ConceptDTO>> getConceptsBySubjectId(@PathVariable Long id, @RequestParam(value = "s", required = false) String sortParam, @AuthenticationPrincipal OAuth2User principal) {
        User user = userService.getUserByEmail(authUtility.getEmail(principal));
    ...
    @Transactional
    @PatchMapping
    public ResponseEntity<Concept> updateConcept(@RequestBody Concept concept, @AuthenticationPrincipal OAuth2User principal){
        User user = userService.getUserByEmail(authUtility.getEmail(principal));
    ...
    @Transactional
    @PutMapping
    public ResponseEntity<Concept> saveConcept(@RequestBody Concept concept, @AuthenticationPrincipal OAuth2User principal) {
    ...
    @DeleteMapping("/{id}")
    @Transactional
    public ResponseEntity<Void> deleteConcept(@PathVariable Long id, @AuthenticationPrincipal OAuth2User principal){
        conceptOwnerCheck(id,  userService.getUserByEmail(authUtility.getEmail(principal)));
    ...
    public ResponseEntity<List<CountPerDate>> getCountOfConceptsPerDate(@AuthenticationPrincipal OAuth2User principal){
        User user = userService.getUserByEmail(authUtility.getEmail(principal));
